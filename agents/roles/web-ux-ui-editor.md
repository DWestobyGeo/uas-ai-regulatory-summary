...
- **State header shows a News count (added v1.5.0).** The `.state-metadata` header strip includes a "News" field showing the total number of related-news items attached across a state's records (see `docs/app.js`, sums `record.news.length`), so a reader can tell at a glance whether a state has any news at all before scrolling.
- **Document intro is a label/value box, not a dense paragraph (corrected v1.5.0).** Every state's source markdown opens with a fixed run of bold-labeled fields on consecutive lines with no blank line between them, which plain Markdown collapses into one dense paragraph. `docs/app.js`'s `buildDocMetaBox()` runs on every state automatically -- driven only by the fixed label set (`Prepared for`, `Research date`, `Version`, `Model / checkpoint`, `Interpretation scope`, `Scope note`), not per-state markup -- and replaces that paragraph with a `.doc-meta-box` label/value list. Fix this class of formatting problem in the shared renderer, not by editing 50 state Markdown files.
- **Boilerplate explainer paragraph removed; process note shortened (v1.5.0).** The "Each authority below is presented as..." paragraph was redundant with the page's own section labels and has been removed from the 29 state Markdown files that had it. The "Process note (Agent Instructions Amendment 5...)" blockquote was shortened to a single-sentence status line (see the same 29 files) -- do not restore the longer version; keep future status notes this short.
- **Related News must render inline in the main narrative, not only in the Source Register accordion (corrected v1.4.0).** The Source Register is a secondary, click-to-expand technical view; the narrative summary (rendered from `summary_markdown`) is the section most readers actually scroll through. A record's news must appear there too, positioned immediately after that record's Practical Interpretation list -- as if it were the next thing on the page, not something a reader has to go looking for. This requires a `<span class="news-anchor" data-record-id="XX-000" hidden></span>` marker placed immediately after the record's `### heading` in the state's `XX_UAS_Regulatory_Summary.md`; `docs/app.js`'s `injectNewsIntoNarrative()` finds it, matches it to the record, and inserts the same news block used in the Source Register right after that record's `.perspectives-list`. `build_data.py` warns at build time if a record has news but no matching anchor is found in its state's narrative markdown -- treat that warning as a defect, not a cosmetic note. The Source Register copy stays too, for readers who came in through that view specifically.
